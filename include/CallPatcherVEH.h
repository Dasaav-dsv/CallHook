...
namespace CallHook {
	...
	public:
		...
		bool patchCall(void* callAddress, void* tgtAddress, uint8_t firstByte, int immOffset) {
			...
			CallPatcher::jobs.emplace_back(PatchJob{ callAddress, static_cast<int>(disp), firstByte, immOffset });
			...
			CallPatcher::write(reinterpret_cast<uint8_t*>(callAddress) + immOffset, static_cast<int>(disp));
			CallPatcher::write(callAddress, firstByte);
	...
	private:
		...
		static LONG WINAPI CallPatcherVEH(EXCEPTION_POINTERS* exInfo) {
			...
			for (auto& job : CallPatcher::jobs) {
				...
				CallPatcher::write(exAddress + job.immOffset, job.tgtDisp);
				CallPatcher::write(exAddress, job.firstByte);
				FlushInstructionCache(GetCurrentProcess(), NULL, 0);
