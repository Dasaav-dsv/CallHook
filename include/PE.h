...
public:
	...
	public:
		...
		/// <summary>
		/// Converts an ibo32 down to an integer.
		/// </summary>
		...
		/// <returns>a pointer calculated from the ibo</returns>
		template <typename T1, typename T2> T1 as(T2* base) noexcept {
		...
		ibo32 operator + (int offset) { ibo32 temp = *this; temp.val += offset; return temp; }
		ibo32 operator - (int offset) { ibo32 temp = *this; temp.val -= offset; return temp; }
		void operator += (int offset) { this->val += offset; }
		void operator -= (int offset) { this->val -= offset; }

		ibo32& operator ++() { ++this->val; return *this; }
		ibo32 operator ++(int) { ibo32 temp = *this; ++this->val; return temp; }
		ibo32& operator --() { --this->val; return *this; }
		ibo32 operator --(int) { ibo32 temp = *this; --this->val; return temp; }
	...
	static PESections* getSectionsWithName(std::string name) {
	...
	/// <param name="sections">: name of the section(s)</param>
	/// <returns>true if ibo is in one of the sections, otherwise false</returns>
	static bool isIbo32InSection(ibo32 ibo, std::string name) {
		return PEParser::isIbo32InSection(ibo, PEParser::getSectionsWithName(name));
	}
